...
- レビュー
  - [X] 2024-02-25 貴島
* 概要
以前[[id:7cacbaa3-3995-41cf-8b72-58d6e07468b1][Go]]の ~errors.Is()~ を調べたので、今度は ~errors.As()~ を調べる。違いは何だろうか。
...
* ユースケース
まずユースケースを見る。 ~fs.PathError~ は ~Unwrap()~ 、 ~As()~ を実装してないので、エラーツリーを辿らず独自判定ロジックで実行しない。
...
- 動機: [[id:20240210T220439][KDOC 86: errors.Is()の比較ロジックを見る]]。IsとAsがあるのはなぜか疑問を持ったので調べた
- 未解決: 代入可能とはどういうことなのか
